...
// ErrorTotalCountJson represents the total count of errors for an agent
// AgentRollup: The rollup identifier of the agent
// AgentID: The unique identifier of the agent
// Value: Total number of errors
type ErrorTotalCountJson struct {
	AgentRollup string `json:"agent_rollup"`
	AgentID     string `json:"agent_id"`
	Value       int    `json:"value"`
}

// CompareErrorTotalCountJsons compares old and new error counts and updates Prometheus metrics
// It tracks changes in error counts between different time periods
func CompareErrorTotalCountJsons(agents, agentsNew []ErrorTotalCountJson, errorTotalCount *prometheus.GaugeVec) {
	// log.Println("\n====== ErrorTotalCountJson COMPARISON REPORT ======")

	// Map to store agent status. Key: AgentRollup_AgentID, Value: Status info
	agentMap := make(map[string]struct {
		agent ErrorTotalCountJson
		inOld bool
		inNew bool
	})
	...
	for _, status := range agentMap {
		if status.inOld && !status.inNew {
			// log.Printf("   Rollup: %s, ID: %s, Value: %d\n", status.agent.AgentRollup, status.agent.AgentID, status.agent.Value)
			errorTotalCount.DeleteLabelValues(
